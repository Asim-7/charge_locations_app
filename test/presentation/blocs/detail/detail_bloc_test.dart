import 'package:charge_locations_app/domain/repositories/charge_location_repository.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:charge_locations_app/presentation/blocs/detail/detail_bloc.dart';
import 'package:charge_locations_app/presentation/blocs/detail/detail_event.dart';
import 'package:charge_locations_app/presentation/blocs/detail/detail_state.dart';
import 'package:charge_locations_app/domain/usecases/fetch_location_detail_usecase.dart';
import 'package:charge_locations_app/data/models/charge_location.dart';
import 'package:charge_locations_app/data/models/evse.dart';

import '../../../data/repositories/mock_repository.dart';

class MockFetchLocationDetailUsecase implements FetchLocationDetailUsecase {
  @override
  Future<ChargeLocation> call(String locationId) async {
    return ChargeLocation(
      address: 'Address 1',
      city: 'Amsterdam',
      country: 'Netherlands',
      latitude: 52.3731,
      longitude: 4.8997,
      evses: [
        Evse(
          evseId: 'EVSE1',
          status: 'AVAILABLE',
          connectorType: 'IEC_62196_T2',
          powerType: 'AC_3_PHASE',
        ),
      ],
    );
  }

  @override
  ChargeLocationRepository get repository => MockRepository();
}

void main() {
  test('LocationDetailBloc emits loaded state on success', () async {
    final fetchLocationDetail = MockFetchLocationDetailUsecase();
    final bloc = LocationDetailBloc(fetchLocationDetail: fetchLocationDetail);

    bloc.add(FetchLocationDetail('123'));

    await expectLater(
      bloc.stream,
      emitsInOrder([isA<LocationDetailLoading>(), isA<LocationDetailLoaded>()]),
    );
  });
}
